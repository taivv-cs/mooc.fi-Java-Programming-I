...
public class BirdDatabaseTest {
    ...
    public void testAddOne() {
        ...
        assertTrue(
            "Program did not print birds name with the One-command. " + message,
            out.contains("XXX")
        );
        assertTrue(
            "Program did not print birds name in Latin with the One-command. " +
                message,
            out.contains("YYY")
        );
        assertTrue(
            "Program did not print number of observations with the One-command. " +
                message,
            out.contains("0")
        );
    }
    ...
    public void bird(String message, String out, String a, String b, int no) {
        ...
        for (String line : lines) {
            if (
                line.contains(a) && line.contains(b) && line.contains("" + no)
            ) {
                ok = true;
            }
        }
        assertTrue(
            "Program did not print birds " +
                a +
                " (" +
                b +
